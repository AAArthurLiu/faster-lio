...
void ImuProcess::UndistortPcl(const common::MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
    ...
    // double imu_beg_time = common::GetSecondsFromHeader(v_imu.front()->header);
    double imu_end_time = common::GetSecondsFromHeader(v_imu.back()->header);
    ...
    for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
        ...
        if (common::GetSecondsFromHeader(tail->header) < last_lidar_end_time_) {
            continue;
        }
        ...
        if (common::GetSecondsFromHeader(head->header) < last_lidar_end_time_) {
            dt = common::GetSecondsFromHeader(tail->header) - last_lidar_end_time_;
        } else {
            dt = common::GetSecondsFromHeader(tail->header) - common::GetSecondsFromHeader(head->header);
        }
        ...
        double &&offs_t = common::GetSecondsFromHeader(tail->header) - pcl_beg_time;
